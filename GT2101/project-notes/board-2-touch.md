...
straight after the Board 1 audit. It found **the timing chain attributed to the wrong
chip** (§2), a **contradiction on the sheet about connector pin 6** that changes what is
safe to touch (§1), and it **confirmed Board 1's display window from this end** — which
is the best outcome the two audits could have had.
...
⚠ **The 10K and the diode were missing from this page until the 5 Sept audit**, and they
matter: pin **5** is taken from the CMOS gate output, but pin **6** is taken from the
gate's *input* node — the same node the diode sits on.
...
- ⚠⚠ **Corrected 5 September 2026.** Two bullets stood here reading *"With Board 3 removed,
  pin 5 drives nothing … Board 3's departure frees the exact signal the Pico wants"* and
  *"It keeps working with Board 3 out of the stack."* **Board 3 is not removed. All five
  boards stay** — settled 29 August 2026.
- ⭐ **So pin 5 is a live net, which is the opposite of what those bullets said.** Board 2
  drives it and board 3 pin 4 receives it. The Pico **listens in parallel** through the
  divider and must never drive it. That is the ⚠ *listen before driving* rule in
...
⚠ **`(= 333 × 40)` was this page's transcription error, not the tracer's.** The layout
footer reads **`= 33,3 × 40`**, which is correct: 33.3 × 40 = 1332. Corrected 5 Sept 2026.
...
### ⭐⭐ What this settles about Board 1 — rewritten 5 September 2026

The first three bullets below were written on 22 August with the wrong chip named. The
audit re-read sheet 2B and the whole display timebase now closes, on both boards at once.
...
- ⚠ **The gate on pin 10 comes from the MC14521, not the 4040.** This page said "the
  4040's Q21 and Q19"; the 4040 is a 12-stage counter and has no Q21. `Q21` and `Q19` are
  on the **MC14521 24-stage divider**, clocked by the **1.048711 MHz crystal signal
  arriving on pin 2** from PCB 4. The arithmetic is exact and settles it beyond doubt:
...
**`Disk-2A-Servo-Control.pdf`** *(was `Disk2AServoControl.pdf`)* — lists **MC14520CP** and **MC14516CP**, neither of which is
...
**`Disk-2B-Power-Driver.pdf`** *(was `Disk2BPowerDriver.pdf`)* — describes LM324/LM358 op-amps, BD675A/BD676A power
...
✅ **Both pages re-read in full on 5 September 2026 and every charge above holds.** A fourth
member of the set has since been caught: `board-1-display/Board-1-Display-Logic-Interface.pdf`
calls the MC14001 an "OR gate" and the MC14011 an "AND gate". **Four checked, four wrong.**
They appear to be written from the galeaudio.com parts list rather than from the boards.
...
| ❓ | Why it matters |
|---|---|
| ~~Which Board 1 pin the LO pulse (pin 6) lands on~~ — ✅ **answered 5 Sept 2026** | Row 2 pad 6 → row 1 pad 2 = **Board 1 physical pin 3, `BLANK`**. See [`flexicon-backplane-map.md`](/GT2101/project-notes/flexicon-backplane-map/) §4 and [`board-1-display.md`](/GT2101/project-notes/board-1-display/) §2a. Left connected, touching the disc blanks the display; whether to break it is an open item in §10 of the backplane map |
...
| ~~Pins 7 and 11 — the two sheets disagree~~ — ✅ **closed 5 Sept 2026** | A naming difference, not a conflict. Directions agree, and `F REF ×40` at 33⅓ **is** 1332 Hz. Pin 11 is an output, driven by the 4040's `Q10` |
| ⚠⚠ **Does pin 6 really swing to −10 V?** | It decides whether that pad is dangerous or ordinary logic, and §1 currently carries a ☠ warning on it. Sheet 2A's circuit and Board 1's pull-up both say no; sheet 2A's own waveform says yes | One meter reading on DC volts, GND to pad 6, tower running, touch the disc (§1) |
| ❓ Where the 4040's clock comes from, and what its `Q10` ÷1024 output really is | Pin 11 is labelled 1332 Hz, but 1.048711 MHz does not divide down to 1332 Hz through a `NAND(Q10, Q9)` reset. Either the crystal figure, the decode or the 1332 label is out by about a factor of two | Nothing depends on it; note it and move on unless pin 11 ever matters |
