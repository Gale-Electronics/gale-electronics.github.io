...
description: "The bootstrap document for the GT2101 Pico controller project — architecture, confirmed findings, open questions, and how the work is recorded."
---

*A working note — part of the GT2101 project's live record, written as the work happens and
including the wrong turns. The polished write-ups live in
...
The bootstrap document for the project. A summary of where things stand, not the record of
every step. Written for whoever picks the work up next, including future me.

**Rewritten 19 August 2026** · **updated 21 August** after the Board 1 display work ·
**22 August** after the Board 2, 3 and 4 studies, when the architecture settled ·
**24 August** for the tinted disc and the suspension · **24 August** again after bench
sessions 5 and 6 — the Helipot is electrically off Board 2, the Pico drives Board 1's green
LED, and the "no driver transistor needed" claim below is corrected.
...
claude.ai project and a `Desktop\GT2101` working folder. On 24 August the claude.ai project
emptied without warning and took twenty-one documents with it; seven were recovered from a
live session's context and the rest were lost. **Do not keep the only copy of anything
outside this repository.**

⚠ The old `Desktop\gale-electronics.github.io-main\` folder was a ZIP download with no
`.git` in it, so nothing in it had ever reached GitHub. It has been superseded and should
not be used.
...
3. Check the other notes in `GT2101/project-notes/` for anything written since — this
   project often moves two or three times in a day, and the documents written *earlier
   today* are the ones most likely to change your answer.
4. Only then start.

On 22 August a full board analysis was delivered without reading two documents written that
same morning, and three of its conclusions were already out of date. Step 3 exists because
of that.
